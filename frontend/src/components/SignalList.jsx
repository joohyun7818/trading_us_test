            ...
            {signals.map((sig) => (
              <tr key={sig.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                ...
                <td className="py-2 px-3">
                  <TypeBadge type={sig.signal_type} />
                </td>
                <td className="py-2 px-3">
                  <ScoreBar score={sig.final_score} label="" />
                </td>
                <td className="py-2 px-3 text-slate-300">{sig.text_score != null ? Number(sig.text_score).toFixed(1) : "—"}</td>
                <td className="py-2 px-3 text-slate-300">{sig.numeric_score != null ? Number(sig.numeric_score).toFixed(1) : "—"}</td>
                <td className="py-2 px-3 text-slate-300">{sig.visual_score != null ? Number(sig.visual_score).toFixed(1) : "—"}</td>
                <td className="py-2 px-3 text-slate-400 text-xs">{sig.created_at ? new Date(sig.created_at).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" }) : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
